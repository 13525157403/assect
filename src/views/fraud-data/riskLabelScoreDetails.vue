<template>
  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      />
      <el-select
        v-model="listQuery.tagCount"
        clearable
        style="width: 120px"
        placeholder="命中标签数"
      >
        <el-option
          v-for="item in tagCountOptions"
          :key="item.value"
          :label="item.label"
          :value="item.value"
        />
    ...
    >
      ...
      <el-table-column label="标签数量" prop="tagCount" align="center" />
      <el-table-column label="标签名称" prop="tagNameSplit" align="center" />
    </el-table>
    <pagination
      v-show="total > 0"
      :total="total"
...
import {
  riskLabelDataCardDetails,
  riskLabelDataCardDownExport,
...
export default {
  ...
  methods: {
    async exportBtn() {
      const res = await riskLabelDataCardDownExport(
        {
          ...
          tagCount: this.listQuery.tagCount,
          tagId: this.listQuery.tagId,
        },
        "tag"
      );
      console.log(res);
      ...
      a.href = url;
      // a.download = "风险标签-累计标签数据-详情.xlsx";
      a.click();
      // 5.释放这个临时的对象url
    ...
    async getList() {
      ...
      const res = await riskLabelDataCardDetails(this.listQuery, "tag");
