...
import { riskLabelDataCardDetails, riskLabelDataCardDownExport } from '@/api/alertInfoDetail'
...
export default {
  ...
  methods: {
    async exportBtn() {
      ...
      a.href = url
      // a.download = '风险标签-累计人数数据-详情.xlsx'
      a.click()
      // 5.释放这个临时的对象url
    ...
    async getList() {
      ...
      const res= await riskLabelDataCardDetails(this.listQuery, 'people')
      this.resultList = res.data.records
      this.total = res.data.total
    },
    async getRuleNameListByMonitorId() {
      // console.log(this.$route.query.monitorId,'id');
