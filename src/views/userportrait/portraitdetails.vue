  ...
  >
    <div class="group">
          ...
          <div>
            <div class="left">
              <el-progress
                type="circle"
                v-if="
                  portraitdata.portrait.showprop ||
                  portraitdata.portrait.showprop === 0
                "
                :percentage="portraitdata.portrait.showprop"
                :format="
                  () => {
                    return progressshow(portraitdata.portrait.proportion);
                  }
                "
                :width="60"
              />
              <div class="left-text">
                <span
                  >{{ portraitdata.portrait.userGroupPeople }} <a>人</a></span
                >
                <span
                  >在 全部用户 中占比{{
                    portraitdata.portrait.proportion
                  }}%</span
                >
              </div>
            </div>
            <div class="right">
              <el-popover placement="top" trigger="click">
                <div class="clk">
                  <el-radio-group
                    v-model="radioCustcreate"
                    size="mini"
                    @change="versionswitching"
                  >
              ...
              <div v-if="radioCustcreate == '1'">
                计算于{{ calculationtime }}
              </div>
              <div v-if="radioCustcreate == '2'">
                计算于{{ createpickdata }}
              </div>
            </div>
          </div>
      ...
      <div
        ...
        element-loading-background="rgba(0, 0, 0, 0.7)"
      >
        <div class="add" v-if="radioCustcreate == '1' || !createpickdata">
            ...
            >
              ...
              <div class="act-cart-1">
                <activecheck
                  ...
                  <a class="act-lab1">线状图</a></activecheck
                >
                <activecheck
            ...
            <div v-else class="from">
              ...
              <div class="act-cart-1">
                <activecheck
                  ...
                  <a class="act-lab1">条状图</a></activecheck
                >
                <activecheck
                  :active="actish_telmp_2[1]"
                  @click="ckeckact2(1, 4)"
                  ><i class="el-icon-refresh-left"></i>
                  <a class="act-lab1">环形图</a></activecheck
                >
                <activecheck
                  :active="actish_telmp_2[2]"
                  @click="ckeckact2(2, 6)"
                  ><i class="el-icon-s-help"></i>
                  <a class="act-lab1">饼图</a></activecheck
                >
              </div>
            </div>
          </el-form-item>
...
export default {
  ...
  methods: {
    //切换版本时刷新图表
    versionswitching() {
      if (this.radioCustcreate == "2") {
        if (!this.createpickdata) {
          return;
        }
        this.CartList.forEach(async (item) => {
          if (item.imageId) {
            item.Cartlod = true;
            let res = await getChartStatisticsData({
              dataTime: this.createpickdata,
              imageChartId: item.imageId,
              options: item.slets,
            }).catch((err) => {
              item.Cartlod = false;
              let { response } = err;
            ...
            item.data = [];
            res.data.forEach((i) => {
              item.data.push({ tagLayerName: i.name, calculateNum: i.people });
            });
          }
        });
      } else if (this.radioCustcreate == "1") {
        this.CartList.forEach(async (item) => {
          if (item.imageId) {
            item.Cartlod = true;
            let res = await getChartStatisticsData({
    ...
    //画像真图表下拉框
    async reallyslets(obj, index) {
      let { slets, imageId } = obj;
      let dataTime = null;
      if (this.radioCustcreate == "2") {
        if (!this.createpickdata) {
          return this.$message.error("指定版本时，时间必选");
        }
        dataTime = this.createpickdata;
      } else {
        dataTime = null;
      ...
      let from = {
        dataTime,
        imageChartId: imageId,
        options: slets,
      };
      this.CartList[index].Cartlod = true;
      let res = await getChartStatisticsData(from).catch((err) => {
      ...
      res.data.forEach((i) => {
        this.CartList[index].data.push({
          tagLayerName: i.name,
          calculateNum: i.people,
        });
      });
    },
    ...
    async creEchodataCustomergroup() {
      let { userGroupId, id: imageId } = this.$route.query;
      this.lod = true;
      const { data } = await findRuleEcho({ userGroupId }).catch((err) => {
        this.lod = false;
        const { response } = err;
        this.$message.error(response.data.message);
        return;
      ...
      this.leftlod = true;
      const res1 = await getUserGroupPeople({ userGroupId }).catch((err) => {
        this.leftlod = false;
        const { response } = err;
        this.$message.error(response.data.message);
        return;
      ...
      this.leftlod = false;
      this.portraitdata.portrait = res1.data;
      let proportion = 0;
      let showprop = 0;
      ...
      this.portraitdata.portrait[`proportion`] = proportion;
      this.portraitdata.portrait[`showprop`] = showprop;
      this.cartlod = true;
      const res2 = await getChartData({ imageId }).catch((err) => {
        this.cartlod = false;
        const { response } = err;
        this.$message.error(response.data.message);
        return;
      ...
      this.cartlod = false;
      this.portraitdata.cartlist = res2.data;
      res2.data.forEach((item) => {
        this.uselabelList.push(item.tagId);
        this.CartList.push({
          CartName: item.charName,
          LabelType: item.tagType,
          LabelName: item.tagId,
          CartType: item.charType,
          data: null,
          slets: "5",
          Cartlod: true,
          imageId: item.id,
    ...
    telmpatedetermine() {
      this.$refs["londfrom"].validate(async (val) => {
        if (val) {
          ...
          this.addcartlod = true;
          let res = await getDataNoAdd(from).catch((err) => {
            this.addcartlod = false;
            const { response } = err;
            this.$message.error(response.data.message);
            return;
          });
    ...
    async addportrait() {
      let from = {
        userGroupId: this.$route.query.userGroupId,
        imageInfoCharts: [],
      ...
      this.lod = true;
      let res = await addImageChart(from).catch((err) => {
        this.lod = false;
        const { response } = err;
        this.$message.error(response.data.message);
        return;
