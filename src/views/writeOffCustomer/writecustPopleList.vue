  ...
  >
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.ruleId" clearable placeholder="命中规则">
        <el-option
          v-for="(val, key, i) in ruleNameList"
          :key="i"
          :label="val.ruleName"
          :value="val.id"
        />
...
import { writeofflisipople, writeoffpopleExport } from "@/api/monitor";
...
export default {
  ...
  methods: {
    async exportBtn() {
      let res = await writeoffpopleExport(this.listQuery);
    ...
    async getList() {
      let { data } = await writeofflisipople(this.listQuery);
    ...
    async getRuleNameListByMonitorId() {
      const ruleNameListResult = await getRuleNameListByMonitorId(
        this.listQuery.monitorId
      );
